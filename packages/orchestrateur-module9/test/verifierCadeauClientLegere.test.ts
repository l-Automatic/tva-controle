...
import { verifierCadeauClientLegere } from '../src/verifierCadeauClientLegere.js';
...
describe('verifierCadeauClientLegere', () => {
  afterAll(async () => {
    await pool.end();
  });
  ...
  it('retourne 0 anomalie ouverte quand la balance ne montre aucun mouvement', async () => {
    ...
    try {
      ...
      const cabinetRes = await provClient.query<{ id: string }>(`SELECT provisioning_create_cabinet($1) AS id`, [
        `Cabinet test verif cadeau client ${Date.now()}`,
      ]);
      ...
      const dossierRes = await provClient.query<{ id: string }>(
        `INSERT INTO dossiers (cabinet_id, nom, regime_tva, logiciel_source, external_company_id, tva_encaissement)
         VALUES ($1, 'Dossier test cadeau client', 'reel_normal', 'pennylane', 'sandbox-cadeau-client', true)
         RETURNING id`,
        [cabinetId]
      );
    ...
    const resultat = await verifierCadeauClientLegere(pool, {
      cabinetId,
      dossierId,
      client,
