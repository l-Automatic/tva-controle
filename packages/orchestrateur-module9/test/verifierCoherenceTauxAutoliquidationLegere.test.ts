...
import { verifierCoherenceTauxAutoliquidationLegere } from '../src/verifierCoherenceTauxAutoliquidationLegere.js';
...
describe('verifierCoherenceTauxAutoliquidationLegere', () => {
  afterAll(async () => {
    await pool.end();
  });

  it('ne fait rien planter et ne signale rien si aucun compte déductible autoliquidation n’est confirmé', async () => {
    ...
    try {
      ...
      const cabinetRes = await provClient.query<{ id: string }>(`SELECT provisioning_create_cabinet($1) AS id`, [
        `Cabinet test verif incoh taux autoliq ${Date.now()}`,
      ]);
      ...
      const dossierRes = await provClient.query<{ id: string }>(
        `INSERT INTO dossiers (cabinet_id, nom, regime_tva, logiciel_source, external_company_id, tva_encaissement)
         VALUES ($1, 'Dossier test incoh taux autoliq', 'reel_normal', 'pennylane', 'sandbox-incoh-autoliq', true)
         RETURNING id`,
        [cabinetId]
      );
    ...
    const resultat = await verifierCoherenceTauxAutoliquidationLegere(pool, {
      cabinetId,
      dossierId,
      client,
