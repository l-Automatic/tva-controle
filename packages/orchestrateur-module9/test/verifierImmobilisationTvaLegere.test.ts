...
import { verifierImmobilisationTvaLegere } from '../src/verifierImmobilisationTvaLegere.js';
...
beforeAll(async () => {
  ...
  try {
    ...
    const cabinetRes = await client.query<{ id: string }>(`SELECT provisioning_create_cabinet($1) AS id`, [
      `Cabinet test verif immob tva ${Date.now()}`,
    ]);
    ...
    const dossierRes = await client.query<{ id: string }>(
      `INSERT INTO dossiers (cabinet_id, nom, regime_tva, logiciel_source, external_company_id, tva_encaissement)
       VALUES ($1, 'Dossier test verif immob tva', 'reel_normal', 'pennylane', 'sandbox-verif-immobtva', true)
       RETURNING id`,
      [CABINET_ID]
    );
...
describe('verifierImmobilisationTvaLegere', () => {
  it('retourne 0 anomalie ouverte quand la balance ne montre aucun mouvement', async () => {
    ...
    const resultat = await verifierImmobilisationTvaLegere(pool, {
