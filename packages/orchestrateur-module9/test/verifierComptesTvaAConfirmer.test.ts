...
import { verifierComptesTvaAConfirmer } from '../src/verifierComptesTvaAConfirmer.js';
...
beforeAll(async () => {
  ...
  try {
    ...
    const cabinetRes = await client.query<{ id: string }>(`SELECT provisioning_create_cabinet($1) AS id`, [
      `Cabinet test verif comptes tva ${Date.now()}`,
    ]);
    ...
    const dossierRes = await client.query<{ id: string }>(
      `INSERT INTO dossiers (cabinet_id, nom, regime_tva, logiciel_source, external_company_id, tva_encaissement)
       VALUES ($1, 'Dossier test verif comptes tva', 'reel_normal', 'pennylane', 'sandbox-verif-tva', true)
       RETURNING id`,
      [CABINET_ID]
    );
...
describe('verifierComptesTvaAConfirmer', () => {
  it('retourne un tableau vide si aucun compte 445xx n’a de mouvement', async () => {
    const client = new PennylaneClient({ token: 'x', fetchImpl: fakeFetchBalanceVide() });
    const resultat = await verifierComptesTvaAConfirmer(pool, {
