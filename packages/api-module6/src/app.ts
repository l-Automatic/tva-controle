...
export function buildApp(pool: Pool): FastifyInstance {
  ...
  // Qualification pour les deux frais véhicule tourisme (10/08) —
  // typeAnomalie dans le corps distingue lequel des deux : "confirme"
  // (correction externe attendue, puis "Vérifier à nouveau") ou "ignore"
  ...
  app.post<{
    ...
    try {
      await avecContexteCabinet(pool, cabinetId, (client) =>
        qualifierFraisVehicule(client, request.params.id, utilisateurId, typeAnomalie, type)
      );
      reply.code(204).send();
    } catch (err) {
      if (err instanceof AnomalieNonQualifiableError) {
  ...
  // "Vérifier à nouveau" — couvre les deux frais véhicule tourisme en un
  // seul appel (10/08). Retire la TVA à tort déduite du calcul brouillon
  ...
  app.post<{
    ...
    return verifierFraisVehiculeLegere(pool, {
      cabinetId,
      dossierId: request.params.dossierId,
      client,
      periodeDebut,
  ...
  // doublon_numerotation_facture (10/08) — pas de qualification
  // préalable requise, comme tva_hotel_a_tort : directement sur les
  // anomalies ouvertes. Aucun ajustement du calcul (ces deux anomalies
  // n'ont jamais d'impact sur la TVA).
  app.post<{
    Params: { dossierId: string };
    Body: { periodeDebut: string; periodeFin: string };
  }>('/dossiers/:dossierId/verifier-numerotation', async (request, reply) => {
    const cabinetId = request.utilisateur!.cabinetId;
    ...
    return verifierNumerotationLegere(pool, {
      cabinetId,
      dossierId: request.params.dossierId,
      client,
      periodeDebut,
  ...
  app.post<{
    Params: { dossierId: string };
    Body: { periodeDebut: string; periodeFin: string };
  }>('/dossiers/:dossierId/verifier-autoliquidation', async (request, reply) => {
    const cabinetId = request.utilisateur!.cabinetId;
    const { periodeDebut, periodeFin } = request.body;
    if (!periodeDebut || !periodeFin) {
      return reply.code(400).send({ erreur: 'periodeDebut et periodeFin sont requis' });
