...
// Fournisseur (401xxx) dont le solde en fin de période sert à corriger la
// TVA déductible en bloc — cf. controles-module4/soldeFournisseurService
// pour l'identification des comptes concernés (fournisseurs de services
// déductible-ABS), et le connecteur pour le calcul du solde lui-même
// (balance cumulée jusqu'à la fin de période, pas juste les mouvements de
// la période — un solde impayé peut dater d'avant).
export interface SoldeFournisseurService {
  compteFournisseur: string;
  soldeTTC: number; // positif = encore dû au fournisseur ; négatif/nul = pas de correction
}

// Remplace, depuis le 04/08, la vérification ligne à ligne du lettrage côté
// achat de service (exigibilite.ts ne traite plus que la collecte) : au lieu
// de suivre facture par facture si chaque achat de service est payé, on
// retire en une fois, du total déductible-ABS de la période, la TVA
// correspondant à ce qui reste dû au fournisseur à la clôture — méthode
// reprise des fichiers de calcul existants de Rami. Hypothèse assumée,
// documentée comme telle : taux national (20%) uniquement, pas de gestion
// des fournisseurs à taux mixte (jugés trop rares pour la complexité que ça
// demanderait — cf. décision du 04/08).
export function corrigerDeductibleParSoldeFournisseurService(
  resultat: ResultatCalculTva,
  soldes: SoldeFournisseurService[]
): ResultatCalculTva {
  const totalARetirer = soldes.reduce((acc, s) => {
    const soldeDu = Math.max(s.soldeTTC, 0);
    // TTC -> TVA à 20% : mêmes maths que integrerRegularisations, appliquées
    // en sens inverse (on retire au lieu d'ajouter).
    return acc + (soldeDu - soldeDu / 1.2);
  }, 0);
  ...
  let trouve = false;
  const lignes: LigneCalculTva[] = resultat.lignes.map((l) => {
    if (l.categorie !== 'deductible_abs') return l;
    trouve = true;
    return { ...l, montant: arrondir(l.montant - totalARetirer) };
  });
  if (!trouve) {
    lignes.push({ categorie: 'deductible_abs', montant: arrondir(-totalARetirer), referencesPieces: [] });
  }
